...
use self::color_model::{ColorModel, DatatypeCode, ModelBPP, RGB};
...
#[test]
fn compare_single_red_pixel_file() {
    let mut single_red_pixel: TgaImage<RGB> = TgaImage::new(1, 1);
    match single_red_pixel.set(0, 0, RGB::set(255, 0, 0)) {
        Ok(_) => {
            let timestamp: String = chrono::Utc::now().format("%Y%m%d%H%M%S").to_string();
            let filename = format!(
                "./tests/generated-files/test-make-single-red-pixel-{}.tga",
                &timestamp
            );

            match single_red_pixel.write_tga_file(&filename) {
                Ok(_) => {
                    if let Result::Ok(fixture) = File::open("./tests/fixtures/single-red-pixel.tga")
                    {
                        if let Result::Ok(generated) = File::open(&filename) {
                            assert_eq!(compare_files(&fixture, &generated), true);
                        }
                    }
                    match std::fs::remove_file(&filename) {
                        Ok(_) => (),
                        Err(_) => panic!("Failed to delete file generated by tests: {}", &filename),
                    }
                }
                Err(e) => println!("Error: {}", e),
            }
        }
        Err(_) => panic!("Failed to set pixel"),
    }
}

#[allow(dead_code)]
fn compare_files(fixture: &File, generated: &File) -> bool {
    let mut fixture = BufReader::new(fixture);
    let mut fixture_buf: Vec<u8> = Vec::new();

    let mut generated = BufReader::new(generated);
    let mut generated_buf: Vec<u8> = Vec::new();

    match fixture.read_to_end(&mut fixture_buf) {
        Ok(fix) => {
            if fix > 0 {
                match generated.read_to_end(&mut generated_buf) {
                    Ok(gen) => return fix == gen,
                    Err(e) => panic!("Could not read generated file. Error: {}", e),
                }
            } else {
                panic!("Empty file received!");
            }
        }
        Err(e) => panic!("Could not read fixture file. Error: {}", e),
    }
}
